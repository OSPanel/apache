...
if !STATUS! == 0 (
  ...
  set OPENSSL_CONFIGURE_OPTS=--prefix=%PREFIX% --libdir=lib --openssldir=%PREFIX%\conf --with-zlib-include=%PREFIX%\include shared zlib-dynamic enable-camellia no-idea no-mdc2 %OPENSSL_BUILD_TYPE% -D"FD_SETSIZE=32768"
...
if !STATUS! == 0 (
  set JANSSON_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DJANSSON_BUILD_SHARED_LIBS=ON -DJANSSON_BUILD_DOCS=OFF -DJANSSON_INSTALL_CMAKE_DIR=lib/cmake/jansson -DCMAKE_POLICY_VERSION_MINIMUM=3.5
...
if !STATUS! == 0 (
  ...
  set APR_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DMIN_WINDOWS_VER=0x0600 -DAPR_HAVE_IPV6=ON -DAPR_INSTALL_PRIVATE_H=ON -DAPR_BUILD_TESTAPR=OFF -DINSTALL_PDB=%INSTALL_PDB% -DCMAKE_C_FLAGS="-DFD_SETSIZE=32768" -DCMAKE_CXX_FLAGS="-DFD_SETSIZE=32768"
...
if !STATUS! == 0 (
  ...
  set APR-UTIL_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DOPENSSL_ROOT_DIR=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DAPU_HAVE_CRYPTO=ON -DAPR_BUILD_TESTAPR=OFF -DINSTALL_PDB=%INSTALL_PDB% -DCMAKE_C_FLAGS="-DFD_SETSIZE=32768" -DCMAKE_CXX_FLAGS="-DFD_SETSIZE=32768"
...
if !STATUS! == 0 (
  set NGHTTP2_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DSTATIC_LIB_SUFFIX=_static -DENABLE_LIB_ONLY=ON -DCMAKE_C_FLAGS="-DFD_SETSIZE=32768" -DCMAKE_CXX_FLAGS="-DFD_SETSIZE=32768"
...
if !STATUS! == 0 (
  ...
  set HTTPD_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DENABLE_MODULES=i -DINSTALL_PDB=%INSTALL_PDB% -DCMAKE_C_FLAGS="-DFD_SETSIZE=32768" -DCMAKE_CXX_FLAGS="-DFD_SETSIZE=32768"
...
if !STATUS! == 0 (
  ...
  set MOD_FCGID_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DINSTALL_PDB=NO -DCMAKE_C_FLAGS="-DFD_SETSIZE=32768" -DCMAKE_CXX_FLAGS="-DFD_SETSIZE=32768"
