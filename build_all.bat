...
if !STATUS! == 0 (
  set "APR-UTIL_CMAKE_OPTS=-DCMAKE_INSTALL_PREFIX=%PREFIX% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DOpenSSL_ROOT_DIR=%PREFIX% -DOpenSSL_INCLUDE_DIR=%PREFIX%\include -DOpenSSL_CRYPTO_LIBRARY=%PREFIX%\lib\libcrypto-3-x64.lib -DOpenSSL_SSL_LIBRARY=%PREFIX%\lib\libssl-3-x64.lib -DAPU_HAVE_CRYPTO=ON -DAPU_HAVE_DSO=ON -DAPRUTIL_WITH_ODBC=ON -DAPRUTIL_WITH_LDAP=ON -DAPR_BUILD_TESTAPR=OFF -DINSTALL_PDB=%INSTALL_PDB%"
